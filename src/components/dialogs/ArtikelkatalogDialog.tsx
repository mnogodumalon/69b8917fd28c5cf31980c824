...
interface ArtikelkatalogDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Artikelkatalog['fields']) => Promise<void>;
  defaultValues?: Artikelkatalog['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function ArtikelkatalogDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: ArtikelkatalogDialogProps) {
  const [fields, setFields] = useState<Partial<Artikelkatalog['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'artikelkatalog');
      await onSubmit(clean as Artikelkatalog['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "artikelnummer": string | null, // Artikelnummer\n  "bezeichnung": string | null, // Bezeichnung\n  "beschreibung": string | null, // Beschreibung\n  "einheit": LookupValue | null, // Einheit (select one key: "stueck" | "stunde" | "tag" | "monat" | "pauschal" | "kilometer" | "liter" | "kilogramm") mapping: stueck=Stück, stunde=Stunde, tag=Tag, monat=Monat, pauschal=Pauschal, kilometer=Kilometer, liter=Liter, kilogramm=Kilogramm\n  "einzelpreis_netto": number | null, // Netto-Einzelpreis (€)\n  "mwst_satz": LookupValue | null, // Mehrwertsteuersatz (select one key: "mwst_0" | "mwst_7" | "mwst_19") mapping: mwst_0=0 %, mwst_7=7 %, mwst_19=19 %\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Artikelkatalog['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Artikelkatalog bearbeiten' : 'Artikelkatalog hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="einzelpreis_netto">Netto-Einzelpreis (€)</Label>
            <Input
              id="einzelpreis_netto"
              type="number"
              value={fields.einzelpreis_netto ?? ''}
              onChange={e => setFields(f => ({ ...f, einzelpreis_netto: e.target.value ? Number(e.target.value) : undefined }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="mwst_satz">Mehrwertsteuersatz</Label>
            <Select
