...
import { ArtikelkatalogDialog } from '@/components/dialogs/ArtikelkatalogDialog';
import { ArtikelkatalogViewDialog } from '@/components/dialogs/ArtikelkatalogViewDialog';
...
const RECHNUNGEN_FIELDS = [
  ...
  { key: 'kunde_vorname', label: 'Vorname des Kunden', type: 'string/text' },
  { key: 'kunde_nachname', label: 'Nachname des Kunden', type: 'string/text' },
  { key: 'kunde_firma', label: 'Firma', type: 'string/text' },
  ...
  { key: 'kunde_plz', label: 'Postleitzahl', type: 'string/text' },
  { key: 'kunde_ort', label: 'Ort', type: 'string/text' },
  { key: 'zahlungsbedingungen', label: 'Zahlungsbedingungen', type: 'lookup/select', options: [{ key: 'sofort', label: 'Sofort fällig' }, { key: 'tage_7', label: '7 Tage netto' }, { key: 'tage_14', label: '14 Tage netto' }, { key: 'tage_30', label: '30 Tage netto' }, { key: 'tage_60', label: '60 Tage netto' }] },
  { key: 'bankverbindung', label: 'Bankverbindung / IBAN', type: 'string/text' },
  { key: 'bemerkungen', label: 'Bemerkungen', type: 'string/textarea' },
];
const RECHNUNGSPOSITIONEN_FIELDS = [
  { key: 'rechnung_zuordnung', label: 'Rechnung', type: 'applookup/select', targetEntity: 'rechnungen', targetAppId: 'RECHNUNGEN', displayField: 'rechnungsnummer' },
  { key: 'positionsnummer', label: 'Positionsnummer', type: 'number' },
  { key: 'artikel_auswahl', label: 'Artikel', type: 'applookup/select', targetEntity: 'artikelkatalog', targetAppId: 'ARTIKELKATALOG', displayField: 'artikelnummer' },
  { key: 'menge', label: 'Menge', type: 'number' },
  { key: 'einheit_position', label: 'Einheit', type: 'lookup/select', options: [{ key: 'stueck', label: 'Stück' }, { key: 'stunde', label: 'Stunde' }, { key: 'tag', label: 'Tag' }, { key: 'monat', label: 'Monat' }, { key: 'pauschal', label: 'Pauschal' }, { key: 'kilometer', label: 'Kilometer' }, { key: 'liter', label: 'Liter' }, { key: 'kilogramm', label: 'Kilogramm' }] },
  { key: 'einzelpreis_netto_pos', label: 'Netto-Einzelpreis (€)', type: 'number' },
  { key: 'mwst_satz_pos', label: 'Mehrwertsteuersatz', type: 'lookup/select', options: [{ key: 'mwst_0', label: '0 %' }, { key: 'mwst_7', label: '7 %' }, { key: 'mwst_19', label: '19 %' }] },
  { key: 'gesamtbetrag_netto', label: 'Gesamtbetrag Netto (€)', type: 'number' },
  { key: 'positionsnotiz', label: 'Notiz zur Position', type: 'string/textarea' },
];
const ARTIKELKATALOG_FIELDS = [
  ...
  { key: 'einheit', label: 'Einheit', type: 'lookup/select', options: [{ key: 'stueck', label: 'Stück' }, { key: 'stunde', label: 'Stunde' }, { key: 'tag', label: 'Tag' }, { key: 'monat', label: 'Monat' }, { key: 'pauschal', label: 'Pauschal' }, { key: 'kilometer', label: 'Kilometer' }, { key: 'liter', label: 'Liter' }, { key: 'kilogramm', label: 'Kilogramm' }] },
  { key: 'einzelpreis_netto', label: 'Netto-Einzelpreis (€)', type: 'number' },
  { key: 'mwst_satz', label: 'Mehrwertsteuersatz', type: 'lookup/select', options: [{ key: 'mwst_0', label: '0 %' }, { key: 'mwst_7', label: '7 %' }, { key: 'mwst_19', label: '19 %' }] },
];

const ENTITY_TABS = [
  { key: 'rechnungen', label: 'Rechnungen', pascal: 'Rechnungen' },
  { key: 'rechnungspositionen', label: 'Rechnungspositionen', pascal: 'Rechnungspositionen' },
  { key: 'artikelkatalog', label: 'Artikelkatalog', pascal: 'Artikelkatalog' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('rechnungen');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'rechnungen': new Set(),
    'rechnungspositionen': new Set(),
    'artikelkatalog': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'rechnungspositionen': return (data as any).rechnungspositionen as Rechnungspositionen[] ?? [];
      case 'artikelkatalog': return (data as any).artikelkatalog as Artikelkatalog[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'rechnungspositionen':
        ...
        lists.artikelkatalogList = (data as any).artikelkatalog ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'rechnungspositionen' && fieldKey === 'rechnung_zuordnung') {
      const match = (lists.rechnungenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.rechnungsnummer ?? '—';
    }
    if (entity === 'rechnungspositionen' && fieldKey === 'artikel_auswahl') {
      const match = (lists.artikelkatalogList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.artikelnummer ?? '—';
    }
    return String(url);
  }, [getLookupLists]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'rechnungspositionen': return RECHNUNGSPOSITIONEN_FIELDS;
      case 'artikelkatalog': return ARTIKELKATALOG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'rechnungspositionen': return {
        create: (fields: any) => LivingAppsService.createRechnungspositionenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateRechnungspositionenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteRechnungspositionenEntry(id),
      };
      case 'artikelkatalog': return {
        create: (fields: any) => LivingAppsService.createArtikelkatalogEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateArtikelkatalogEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteArtikelkatalogEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'rechnungspositionen' || dialogState?.entity === 'rechnungspositionen') && (
        <RechnungspositionenDialog
          open={createEntity === 'rechnungspositionen' || dialogState?.entity === 'rechnungspositionen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'rechnungspositionen' ? handleUpdate : (fields: any) => handleCreate('rechnungspositionen', fields)}
          defaultValues={dialogState?.entity === 'rechnungspositionen' ? dialogState.record?.fields : undefined}
          rechnungenList={(data as any).rechnungen ?? []}
          artikelkatalogList={(data as any).artikelkatalog ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Rechnungspositionen']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Rechnungspositionen']}
        />
      )}
      {(createEntity === 'artikelkatalog' || dialogState?.entity === 'artikelkatalog') && (
        <ArtikelkatalogDialog
          open={createEntity === 'artikelkatalog' || dialogState?.entity === 'artikelkatalog'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'artikelkatalog' ? handleUpdate : (fields: any) => handleCreate('artikelkatalog', fields)}
          defaultValues={dialogState?.entity === 'artikelkatalog' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Artikelkatalog']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Artikelkatalog']}
        />
      )}
      ...
      {viewState?.entity === 'rechnungspositionen' && (
        <RechnungspositionenViewDialog
          open={viewState?.entity === 'rechnungspositionen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'rechnungspositionen', record: r }); }}
          rechnungenList={(data as any).rechnungen ?? []}
          artikelkatalogList={(data as any).artikelkatalog ?? []}
        />
      )}
      {viewState?.entity === 'artikelkatalog' && (
        <ArtikelkatalogViewDialog
          open={viewState?.entity === 'artikelkatalog'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'artikelkatalog', record: r }); }}
        />
      )}
