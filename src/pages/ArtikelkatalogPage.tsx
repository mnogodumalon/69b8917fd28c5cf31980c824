...
export default function ArtikelkatalogPage() {
  const [records, setRecords] = useState<Artikelkatalog[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Artikelkatalog | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Artikelkatalog | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Artikelkatalog | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getArtikelkatalog());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Artikelkatalog['fields']) {
    await LivingAppsService.createArtikelkatalogEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Artikelkatalog['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateArtikelkatalogEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteArtikelkatalogEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Artikelkatalog im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Artikelkatalog suchen..."
          value={search}
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell>{record.fields.bezeichnung ?? '—'}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Artikelkatalog. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
